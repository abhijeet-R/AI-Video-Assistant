...
with st.sidebar:
    ...
    if st.session_state.pipeline_done:
        st.markdown("---")
        st.markdown(
            '<span class="badge badge-green">Pipeline Status</span>', unsafe_allow_html=True)
...
if st.session_state.result:
    ...
    with col2:
        with st.expander("📝 Full Transcript", expanded=False):
            st.markdown(
                f'<div class="transcript-box">{r["transcript"]}</div>', unsafe_allow_html=True)
    ...
    with chat_col1:
        user_input = st.text_input(
            "Your question", placeholder="Ask question", label_visibility="collapsed")
    with chat_col2:
